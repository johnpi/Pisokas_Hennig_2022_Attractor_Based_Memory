...
#import bee_simulator as animal_simulator
import ant_simulator as animal_simulator
...
# Values better approximating ant path dynamics
turn_sharpness = 1.0 # Originally was 1.0
default_acc = 0.08
default_drag = 0.15
...
def generate_route(T=1500, mean_acc=default_acc, drag=default_drag,
                   ...
                   vary_speed=False):
    """Generate a random outbound route using animal_simulator physics.
    ...
    for t in range(1, T):
        headings[t], velocity[t, :] = animal_simulator.get_next_state(
...
def CPU4_amplitude(CPU4):
    """ Neurally plausible estimate of peak to peak. About 4 times smaller than the actual. """
    step = 4
    mag = np.clip(CPU4[0:11] - CPU4[step:step+11], 0.0, 1.0)
    mag_sum = np.sum(mag, axis=0)/12
    return mag_sum

def CPU4_amplitude2(CPU4):
    """ Alternative neurally plausible estimate of peak to peak. About 4 times smaller than the actual. """
    step = 4
    mag = np.clip((CPU4[0:4] - CPU4[4:8]) + (CPU4[8:12] - CPU4[12:16]), 0.0, 1.0)
    mag_sum = np.sum(mag, axis=0)/2
    return mag_sum

def CPU4_amplitude_math(CPU4):
    """ The actual peak to peak """ 
    mag = np.clip(np.max(CPU4, axis=0) - np.min(CPU4, axis=0), 0.0, 1.0)
    return mag
...
def homing(T, tb1, memory, cx, acceleration=default_acc, drag=default_drag,
           ...
           filtered_steps=0.0, noise_turn=0.0, noise_turn_slope=125.0):
    ...
    for t in range(1, T + 1):
        ...
        rand_accel_factor = 1.0

        # Max turning noise level is a function of memory amplitude
        noise_turn_factor = turn_noise_level(CPU4_amplitude_math(memory), slope=noise_turn_slope, y_max=1.0, y_min=0.0)
        motor_noise = noise_turn_factor*noise_turn
        
        headings[t], velocity[t, :] = animal_simulator.get_next_state(
            headings[t - 1], velocity[t - 1, :], rotation, acceleration*rand_accel_factor, drag, motor_noise)
...
def run_trial(route=None, T_outbound=1500, T_inbound=1500, acc_out=default_acc,
              ...
              cooling_treatment_method=None, 
              noise_turn=0.0, 
              noise_turn_slope=125.0, 
              random_homing_start_velocity=True):
    ...
    cooling_treatment_method
    noise_turn 
    noise_turn_slope 
    random_homing_start_velocity : This is a new formal parameter. The 
    original default behaviour would be produced by setting it to False. 
    With value True every run of the homing starts with a random generated 
    initial velocity. 
    """
    ...
    if cooling_treatment_method is not None: 
        memory = cx.cpu4_freezing(memory, cooling_treatment_method)
    ...
    if random_homing:
        ...
        if random_homing_start_velocity:
            # Create a random starting velocity vector
            start_velocity = np.array([np.random.normal(0, 0.01), np.random.normal(0, 0.01)])
            # Limit the velocity values to the -0.01-0.01 range
            start_velocity[start_velocity>0.01]  =  0.01
            start_velocity[start_velocity<-0.01] = -0.01
            # Calculate the starting heading. Note that Tom's code uses the order v=[y, x]
            start_heading = np.arctan2(start_velocity[0], start_velocity[1])
            if start_heading < 0:
                start_heading = start_heading + 2 * np.pi
            # Set velocity to 0
            start_velocity = np.array([0.0, 0.0])
        else:
            # Use the velocity of the last outbound step
            start_velocity=v_out[-1]
            start_heading=h_out[-1]
            
        h_in, v_in, log_in = homing(
            T=T_inbound, tb1=tb1, memory=memory, cx=cx,
            acceleration=acc_in, current_heading=start_heading,
            current_velocity=start_velocity, turn_sharpness=turn_sharpness, logging=logging, bump_shift=bump_shift,
            filtered_steps=filtered_steps, drag=drag, noise_turn=noise_turn, noise_turn_slope=noise_turn_slope)
...
def load_route(filename='route.npz', data_path=DATA_PATH):
...
def save_route(h, v, cx_log, filename='route.npz', data_path=DATA_PATH):
...
def load_dataset(T_outbound, T_inbound, noise, N, data_path=DATA_PATH, **kwargs):
...
def save_dataset(H, V, cpu4_snapshot, T_outbound, T_inbound, noise, N, data_path=DATA_PATH,
                 **kwargs):
    filename = generate_filename(T_outbound, T_inbound, noise, N,
                                 **kwargs)
